...
public class PacmanUIValidationView extends ViewPart {
	 ...
	 */
	private List<ValidationRow> _rows;
	...
	public void createPartControl(Composite p_parent) {
		...
		_viewer.addSelectionChangedListener(event -> {
			...
			ValidationRow row = (ValidationRow) selection.getFirstElement();
	...
	/**
	 * Met en surbrillance la ligne correspondant à l'objet {@link ValidationRow}
	 ...
	 */
	private void highlightSelectedRow(final ValidationRow p_row) {
		for (TableItem item : _viewer.getTable().getItems()) {
	...
	private void createColumns() {
		...
		Comparator<ValidationRow>[] comparators = (Comparator<ValidationRow>[]) new Comparator[] {
				Comparator.comparing((ValidationRow r) -> r._rule == null ? "" : r._rule),
				Comparator.comparing((ValidationRow r) -> r._objectName == null ? "" : r._objectName),
				Comparator.comparing((ValidationRow r) -> r._error == null ? "" : r._error),
				Comparator.comparing((ValidationRow r) -> r._solution == null ? "" : r._solution) };
		...
		for (int i = 0; i < titles.length; i++) {
			...
			col.setLabelProvider(new ColumnLabelProvider() {
				...
				public String getText(Object element) {
					ValidationRow row = (ValidationRow) element;
			...
			tableCol.addListener(SWT.Selection, e -> {
				...
				_viewer.setComparator(new ViewerComparator() {
					...
					public int compare(Viewer viewer, Object e1, Object e2) {
						int result = comparators[columnIndex].compare((ValidationRow) e1, (ValidationRow) e2);
	 ...
	 */
	private void navigateToElement(ValidationRow p_row) {
	 ...
	 *               configurer la vue.
	 */
	void setLinkingEnabled(final IWorkbenchPage p_page) {
		Display.getDefault().asyncExec(() -> {
	 ...
	 */
	public void setRows(final List<ValidationRow> p_rows) {
		this._rows = p_rows;
